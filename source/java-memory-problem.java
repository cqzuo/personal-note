...
JAVA内存泄漏问题
作者: 王峰, 　出处:构客网,　责任编辑: 李春禹,　2007-11-13 10:19
　　Java的一个重要优点就是通过垃圾收集器(Garbage Collection，GC)自动管理内存的回收，程序员不需要通过调用函数来释放内存。Java的一个重要优点就是通过垃圾收集器(Garbage Collection，GC)自动管理内存的回收，程序员不需要通过调用函数来释放内存。
...
能不能告诉我啊.呵呵
hardsonxu：另外补充一下：
其它java内存池漏情况并不象你所说的这么严重。
你上面所说的很多论点，其实就两点：
1、过期引用
2、物理连接未释放
过期引用情况也不多见，比如压栈出栈(Stack类)就可能出现（参见《effective java》
更多的情况是物理连接未关闭导致内存池漏
hardsonxu：(1)好的编码习惯。最基本的建议就是尽早释放无用对象的引用，大多数程序员在使用临时变量的时候，都是让引用变量在退出活动域后，自动设置为null。在使用这种方式时候，必须特别注意一些复杂的对象图，例如数组、列、树、图等，这些对象之间有相互引用关系较为复杂。对于这类对象，GC 回收它们一般效率较低。如果程序允许，尽早将不用的引用对象赋为null。另外建议几点：
在确认一个对象无用后，将……
----------------------------------------------------------------------------------
  JAVA 语言内存泄漏问题收藏
新一篇: 程序员面试之葵花宝典 | 旧一篇: 2007中国软件技术英雄会之一 创业英雄论坛
 1 引言
...
3.1 Java 中内存泄漏与C++的区别
    在Java 中，内存泄漏就是存在一些被分配的对象，这些对象有下面两个特点，首先，这些对象是可达的，即在有向图中，存在通路可以与其相连；其次，这些对象是无用的，即程序以后不会再使用这些对象。如果对象满足这两个条件，这些对象就可以判定为Java 中的内存泄漏，这些对象不会被GC 所回收，然而它却占用内存。在C++中，内存泄漏的范围更大一些。有些对象被分配了内存空间，然后却不可达，由于C++中没有GC，这些内存将永远收
...
发表于 @ 2007年05月13日 22:06:00|评论(2)|编辑

新一篇: 程序员面试之葵花宝典 | 旧一篇: 2007中国软件技术英雄会之一 创业英雄论坛评论
#hardsonxu 发表于2007-09-25 00:23:07  IP: 116.21.63.*
(1)好的编码习惯。最基本的建议就是尽早释放无用对象的引用，大多数程序员在使用临时变量的时候，都是让引用变量在退出活动域后，自动设置为null。在使用这种方式时候，必须特别注意一些复杂的对象图，例如数组、列、树、图等，这些对象之间有相互引用关系较为复杂。对于这类对象，GC 回收它们一般效率较低。如果程序允许，尽早将不用的引用对象赋为null。另外建议几点：
在确认一个对象无用后，将其所有引用显式的置为null；

上述说法是错误的。
原因有两点：
1、在一个方法内，jvm只有在方法结束后才会把方法内的失去引用的对象纳入回收机制。不管你什么时候在方法内把一个不再使用的对象设置为null，虽然原引用对象不可达，jvm也不会马上把它纳入回收机制。
2、在程序里如此这般地使用null以试图保证对象尽早或安全地回收，好象使人又回到了c++时代，不仅没必要，同时使得代码累赘又难看，完成违背了java语言设计的初衷。#hardsonxu 发表于2007-09-25 00:31:31  IP: 116.21.63.*
另外补充一下：
其它java内存池漏情况并不象你所说的这么严重。
你上面所说的很多论点，其实就两点：
1、过期引用
